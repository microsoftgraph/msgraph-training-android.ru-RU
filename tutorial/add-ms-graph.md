...
Начните с расширения `GraphHelper` класса, чтобы добавить функцию, чтобы получить события пользователя. Откройте файл **графхелпер** и добавьте приведенные ниже `import` операторы в начало файла.
...
Добавьте в `GraphHelper` класс следующие функции.
...
Теперь обновите `CalendarFragment` их, чтобы использовать эти новые функции. Добавьте приведенные `import` ниже операторы в начало файла **календарфрагмент** .
...
Добавьте в `CalendarFragment` класс следующие элементы.
...
Теперь добавьте в `CalendarFragment` класс следующие функции для скрытия и отображения индикатора выполнения, а также для предоставления обратного вызова для `getEvents` функции в. `GraphHelper`
...
Наконец, переопределите `onCreate` функцию в `GraphHelper` классе, чтобы получить события пользователя из Microsoft Graph.
...
Теперь вы можете заменить дамп JSON на какой-то способ отобразить результаты в удобном для пользователя виде. Сначала замените элемент `TextView` **app/res/layout/фрагмент_календар. XML** на `ListView`.
...
Затем создайте макет для каждого элемента в файле `ListView`. Щелкните правой кнопкой мыши папку **app/res/layout** и выберите команду **создать**, а затем — **файл ресурсов макета**. ПриСвойте файлу `event_list_item`имя, измените **корневой элемент** на `RelativeLayout`, а затем нажмите кнопку **ОК**. Откройте файл **евент_лист_итем. XML** и замените его содержимое на приведенный ниже код.
...
Теперь создайте настраиваемый адаптер списка для `ListView`. Это необходимо, чтобы создать представления для элементов списка и сопоставить их поля `Event` с соответствующими `TextView` элементами в представлении.

Щелкните правой кнопкой мыши папку **app/Java/com. example. графтуториал** и выберите команду **создать**, а затем — **класс Java**. НаЗовите класс `EventListAdapter` и нажмите кнопку **ОК**. Откройте файл **евентлистадаптер** и замените его содержимое на приведенный ниже код.
...
Наконец, обновите `CalendarFragment` класс, чтобы он `EventListAdapter` использовался для `ListView`инициализации. Откройте класс **календарфрагмент** и добавьте приведенную ниже функцию в класс.
...
Добавьте следующую строку кода в `success` переопределение после `mEventList = iEventCollectionPage.getCurrentPage();` строки.

```java
addEventsToList();
```

Запустите приложение, войдите в систему и нажмите элемент навигации **** по календарю. Вы должны увидеть список событий.
